...
@Controller
@RequestMapping(value = "/api/adendasCas")
public class AdendasCasController {
    static final Logger logger = LoggerFactory.getLogger(AdendasCasController.class);

    @Autowired
    AdendasCasService adendasCasService;

    //Para el udid del correo q ingreso al sistema
    @RequestMapping(method = RequestMethod.GET, produces = "application/json", value = "/traerUdid/{correo}/unico")
    @ResponseStatus(HttpStatus.ACCEPTED)
    @ResponseBody
    public Servidor traer(@PathVariable(value = "correo") String correo) {
        Servidor servidor = new Servidor();
        System.out.println("correoooo!!!"+correo);
        servidor.setCorreo(correo);

        List<Servidor> dependencias = adendasCasService.traer(servidor);
        if (dependencias.size() == 0)
            return null;
        return dependencias.get(0);
    }

    //Para listar los contratos
    @RequestMapping(method = RequestMethod.GET, produces = "application/json", value = "/contratos/{udid}")
    @ResponseStatus(HttpStatus.ACCEPTED)
    @ResponseBody
    public List<Servidor> listarContratos(@PathVariable(value = "udid") Integer udid){

        System.out.println("Listar Contratos!!!!");
        List<Servidor> cont = adendasCasService.listarContratos(udid);
        return cont;
    }

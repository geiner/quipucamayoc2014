...
public interface AdendasCasMapper {


    //para traer el udid de la tabla tb_hist_usu_perf

    @Select(value =" select ud_id  udid from qprodataquipu.tb_hist_usu_perf  " +
            "  where t_mail = #{ser.correo}   " )

    @Results(value = {@Result(javaType = Servidor.class),

            @Result(property = "udid", column = "udid")
    })
    List<Servidor> traer(@Param("ser") Servidor servidor);



    //*Para listar los contratos CAS *//*
    @Select(value = "select aa.ser_cod dni, cc.ser_ape_pat pat, cc.ser_ape_mat mat, cc.ser_nom nom, aa.num_serest numserest,  " +
            " aa.t_contrato_numero num, aa.c_cargo_id idcar,bb.t_cargo_descripcion descar, to_char((aa.f_contratoadenda_fecha_ini),'DD/MM/YYYY') fini,  " +
            " to_char((aa.f_contratoadenda_fecha_fin),'DD/MM/YYYY') ffin, aa.n_monto monto, aa.t_contratoadenda_tipo tipo  " +
            " from (qpdatagestion.tb_contratos_adendas  aa left join qpdatagestion.tb_cargos  bb on  bb.c_cargo_id=aa.c_cargo_id)  " +
            " left join datapersuel.servidor cc on cc.ser_cod=aa.ser_cod  " +
            " where aa.t_contratoadenda_estado='ACTIVO' and aa.n_ud_id=#{udid} and aa.t_adenda_numero is null " )
    @Results(value = {
            @Result(javaType = Servidor.class),
            @Result(property = "numDoc", column = "dni"),
            @Result(property = "paterno", column = "pat"),
            @Result(property = "materno", column = "mat"),
            @Result(property = "nombre", column = "nom"),
            @Result(property = "num_serest", column = "numserest"),
            @Result(property = "contrato", column = "num"),
            @Result(property = "idcargo", column = "idcar"),
            @Result(property = "descargo", column = "descar"),
            @Result(property = "fechini", column = "fini") ,
            @Result(property = "fechfin", column = "ffin") ,
            @Result(property = "monto1", column = "monto"),
            @Result(property = "destipo", column = "tipo")

    })
    List<Servidor> listarContratos(@Param("udid") Integer udid)throws DataAccessException;
  ...
  /*  *//*Para listar los servidores CAS *//*
    ...
    *//*Para listar las plazas *//*
    @Select(value = "SELECT  P.T_PLAZA_CODIGO COD_PLAZA, p.c_plaza_id as id_plaza,P.N_PLAZA_ESTADO ESTADO_PLAZA, se.ser_cod as DNI ,se.ser_num_ruc as ruc, " +
            ...
            "        ca.n_monto AS monto, ca.t_contratoadenda_tipo as tipo,CA.t_contratoadenda_estado ESTADO_CONTRATO, CA.F_CONTRATOADENDA_FIN_LABORAL FECHFIN_LABORAL  " +
